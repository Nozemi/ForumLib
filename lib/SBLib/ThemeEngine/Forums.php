<?php
    ...
    class Forums extends MainEngine {
        ...
        public function __construct(MainEngine $engine) {
            parent::__construct($engine->_SQL, $engine->getName(), $engine->_Config);
            $this->_engine = $engine;
        }
                    ...
                    case 'adminMenu':
                        ...
                        if(!empty($_SESSION['user'])) {
                            ...
                            if(isset($user->group->admin)) {
                                $html = $this->parseCategory($this->_engine->getTemplate('admin_categories', 'admin'), $_category);
                            }
                        }
                ...
                switch($template[1]) {
                    ...
                    case 'threads':
                        ...
                        if(!empty($_topic->threads)) {
                            foreach($_topic->threads as $thread) {
                                $html .= $this->parseForum($this->_engine->getTemplate('thread_view', 'forums'), $thread);
                            }
                        } else {
                    ...
                    case 'adminMenu':
                        ...
                        if(!empty($_SESSION['user'])) {
                            ...
                            if(isset($user->group->admin)) {
                                $html = $this->_engine->getTemplate('admin_topic', 'admin');
                            }
                        }
                ...
                switch($template[1]) {
                    ...
                    case 'manage':
                        ...
                        if(!empty($_SESSION['user'])) {
                            ...
                            || $user->group->admin) {
                                $html = $this->parsePost($this->_engine->getTemplate('post_view_manage', 'forums'), $_post);
                            }
                        }
